...
#define CONFIG_EXTRA_ENV_SETTINGS	"kernel_addr=0x2000000\0"	\
					...
					"kernel_image_size=0xbcc800\0"	\
					"dtb_size=0x2b92\0"		\
					"ramfs_image_size=0x14e0000\0"	\
					"hostname=uDPU\0"		\
					"ramfs_addr=0x3000000\0"	\
					"ramfs_name=openwrt-armada-3720-db-Image-initramfs\0"		\
					...
					"ramfs_addr -;fi\0"		\
					"mtdparts=spi0.0:2m(Bootloader),11m(Kernel),16k(DTB),"\
					"-(RootFileSystem)\0"	\
					"flashboot=sf probe 0; sf read ${kernel_addr} "\
					"${offset_kernel} ${kernel_image_size}; sf read ${fdt_addr} "\
					"${offset_dtb} ${dtb_size}; booti ${kernel_addr} - "\
                                        "${fdt_addr}\0"	\
					"console=" CONFIG_DEFAULT_CONSOLE "\0"\
					"extra_params=raid=noautodetect\0" \
					"root=root=/dev/mtdblock3 rootfstype=jffs2 rw rootwait ${mtdparts}\0"	\
